...
#define instantiate(type, dim)  template void Simulation<type,dim>::store_LMU(Eigen::Array<type, Eigen::Dynamic, Eigen::Dynamic> *); \
  template void Simulation<type,dim>::LMU(int, int, Eigen::Array<unsigned long, Eigen::Dynamic, 1>); \
  template void Simulation<type,dim>::calc_LDOS();
